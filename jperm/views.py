...
from jumpserver.celerytasks import task_ansible_role_push
...
def perm_role_push(request):
    ...
    if request.method == "POST":
        push_task = {}
        push_task['assets'] = request.POST.getlist("assets")
        push_task['asset_groups'] = request.POST.getlist("asset_groups")
        push_task['use_password'] = request.POST.get("use_password")
        push_task['use_publicKey'] = request.POST.get("use_publicKey")
        task_ansible_role_push.delay(user,push_task,role)
