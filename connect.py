...
class Nav(object):
    ...
    def reload_user_perm(self,user):
	self.__init__(user)
    ...
    def print_nav():
        ...
        5) 输入 \033[31mG/g\033[0m\033[0m + \033[31m组ID\033[0m 显示该组下主机. 如: g1
    ...
    def print_search_result(self):
        ...
        line = '[%-3s]  %-16s %-5s  %-' + str(hostname_max_length) + 's %-12s [%-3s]  %s'
    ...
    try:
        while True:
            try:
                ...
                sys.exit(0)
            if option in ['P', 'p', '\n', '']:
		nav.reload_user_perm(login_user)
                nav.search()
