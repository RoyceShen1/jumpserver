...
def checkPassword(pwd):
    ...
    OK = lenOK + upperOK + lowerOK + numOK + symbolOK
    return (OK>=4)

# 新增用户提交申请和超级管理员处理申请的视图
@require_role(role='super')
def request_deal(request):
    applications = Application.objects.filter(processed = False)
    header_title, path1, path2 = '查看申请', '用户管理', '申请列表'
    return my_render('juser/request_deal.html', locals(), request)
...
def request_process(request):
    ...
    if id:
        ...
        user = a.applicant
        title = '申请已处理'
        msg = '你好,你的申请已经处理完成。申请内容如下:%s'%(a.description)
        mail_notify(title,msg,user)
